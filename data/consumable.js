dat32 = {
	"items": {
		"item": [
			{ // reodering these based on my preferred sidebar order XD
				"_id": "905273706",
...
window.grabConsumeName = function(ID = "(???)") { // id name fallback
  ...
  if (typeof dat32 === 'object' && dat32.items?.item && Array.isArray(dat32.items.item)) {
    ...
    return match?._name ?? ID; // if it cant get the name at any point or something goes wrong just return ID
  }

  return ID; // IF something goes wrong just return ID
}
